...
export function Learn() {
  ...
  return (
    <Page title="Learn">
      <Muted>Tracks → modules. Locked modules show a preview; ask Claude to teach a topic and it unlocks here.</Muted>
        ...
        {mods.map((m) => {
          ...
          return (
            <ListRow key={m.id} to={`/learn/${m.track}/${m.id}`} right={pill}>
              <span className={m.status === 'preview' ? 'opacity-70' : ''}>{m.title}</span>
