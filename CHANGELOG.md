...
### Changed
- crossStack cards and table cells break long tokens (`overflow-wrap: anywhere`), so mentor snippets like `Model::whereKey($id)->lockForUpdate()->first()` no longer push the page wide on phones.
